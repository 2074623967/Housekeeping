...
5. 同一订单在预付单未失效且支付单未收口时重复创建预付单，必须复用原预付单与支付单。
6. 同一预付单在支付单已进入 `WAIT_CALLBACK`、`SUCCESS` 或 `CLOSED` 后重复提交，必须直接幂等返回，不再新增路由和尝试记录。
...
| PAY-TC-016 | 重复提交支付幂等 | 对同一预付单连续两次调用 `POST /api/payments/submit` | 第二次直接返回当前预付单，不新增路由和支付尝试 | P0 |
