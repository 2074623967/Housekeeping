...
5. `FAILED` outbox 由定时任务按 `next_retry_at` 定向检索并批量重投，默认每 `5` 分钟运行一次、每批最多 `100` 条；重投仍由同一 relay 回写成功或下一次失败时间。
...
1. 订阅 `payment.success.v1` 和 `clearing.generated.v1`。当前已实现可开关的 `accounting.payment-success` 与 `accounting.clearing-generated` AMQP consumer。
2. 以 `accountNo + bizType + bizNo` 做账务流水幂等；业务成功后才手动 ACK。
3. `payment.success.v1` 消费异常按 `x-retry-count` 发送到 `payment.trade.retry`，达到上限发送到 `payment.trade.dlq`；`clearing.generated.v1` 使用隔离的 `clearing.trade.retry / clearing.trade.dlq` 路由；重投失败时均 NACK 原消息并 requeue。
4. HTTP 接口继续保留作为未部署 broker 环境的兼容入口。
