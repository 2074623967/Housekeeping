...
- `payment-core` 当前技术栈是 `Spring Boot 2.7.18 + MyBatis + MySQL`，尚未引入 MQ 依赖。
- `payment-core` 当前支付事件以 `t_payment_event` 作为支付事实台账，关键字段只有 `event_no / event_type / payment_order_id / biz_no / event_payload`。
- `payment-core` 当前通过 `payment.downstream.clearing.payment-success-url` 和 `payment.downstream.accounting.payment-success-url` 直接发 HTTP 下游，不是消息代理。
- `clearing-system -> settlement-system / accounting-system` 当前也还是本地 HTTP 下游派发。
...
1. 没有真实消息代理。
2. 没有 broker 级投递确认、消费确认、消费组、重试队列和死信队列。
...
1. 订阅 `payment.success.v1`。
2. 先以 `paymentOrderId` 做消费幂等，再执行业务清分。
3. 清分成功后新增 `CLEARING_GENERATED` outbox，再由本地 relay 投递到 `clearing.trade`。
4. 失败时按 retry / dlq 路由，不能直接丢失。

### 6.3 settlement-system

1. 订阅 `clearing.generated.v1`。
2. 以 `clearingNo + targetType + targetNo` 做消费幂等。
3. 若消费成功，只生成结算事实，不在 MQ 消费线程里直接做复杂出款。
4. 出款仍由结算批次或任务中心驱动，避免 MQ 消费线程承担长事务。

### 6.4 accounting-system

1. 订阅 `payment.success.v1` 和 `clearing.generated.v1`。
2. 以 `accountNo + bizType + bizNo` 做账务流水幂等。
3. 事件消费成功后写事件台账、账务流水和余额快照。
4. 如果消费异常，不允许部分写成功后直接 ACK。
